...
class MelDataset(torch.utils.data.Dataset):
    def __init__(self, data_list, sr=48000, data_augmentation=False, validation=False, verbose=True):
        ...
        self.sr = sr
        self.to_melspec = torchaudio.transforms.MelSpectrogram(sample_rate=self.sr, **MEL_PARAMS)
...
class Collater(object):
    ...
    def __call__(self, batch):
        ...
        if self.max_mel_length > self.min_mel_length:
            random_slice = (
                np.random.randint(
                    self.min_mel_length // self.mel_length_step, 1 + self.max_mel_length // self.mel_length_step
                )
...
def build_dataloader(
    path_list, validation=False, batch_size=4, num_workers=1, device="cpu", collate_config={}, dataset_config={}
):
